...
INSERT IGNORE INTO `guarantor_settings` (`setting_key`, `setting_value`, `description`) VALUES
    ('min_guarantors',                '2',     'Minimum number of guarantors required'),
    ('max_guarantors',                '4',     'Maximum number of guarantors allowed'),
    ('min_coverage_percentage',       '100',   'Minimum coverage percentage required'),
    ('guarantor_must_be_member',      '1',     'Guarantor must be a registered member'),
    ('guarantor_max_active_guarantees','3',    'Maximum active guarantees a member can give'),
    ('guarantor_min_membership_days', '30',    'Minimum membership days before can be guarantor'),
    ('allow_self_guarantee',          '0',     'Allow member to guarantee their own loan'),
    ('guarantor_liability_type',      'joint', 'Liability type: joint or several'),
    ('auto_debit_from_guarantor',     '1',     'Auto debit from guarantor if borrower defaults'),
    ('notify_guarantor_on_default',   '1',     'Send notification to guarantor on default');
...
INSERT IGNORE INTO `accounting_settings` (`setting_key`, `setting_value`, `description`) VALUES
    ('default_cash_account',        '1',           'Default cash account ID'),
    ('default_bank_account',        '1',           'Default bank account ID'),
    ('fiscal_year_start',           '04-01',       'Fiscal year start month-day'),
    ('auto_close_fiscal_year',      '0',           'Auto close fiscal year'),
    ('depreciation_method',         'straight_line','Default depreciation method'),
    ('rounding_precision',          '2',           'Decimal places for rounding'),
    ('enable_double_entry',         '1',           'Enable double-entry bookkeeping'),
...
CREATE TABLE IF NOT EXISTS `bank_imports` (
    ...
    `status`         ENUM('pending','matched','ignored','error') DEFAULT 'pending',
