...
class Check_library {
     ...
     * @return array ['success' => bool, 'error' => string or null]
     */
    public function save_check_details($application_id, $check_data, $created_by = null) {
        if (!$this->table_exists()) {
            return ['success' => false, 'error' => 'Missing database table: loan_check_details. Run the schema migration or create the table in MySQL.'];
        }
        $insert_data = array(
        ...
        if ($this->CI->db->insert('loan_check_details', $insert_data)) {
            return ['success' => true, 'error' => null];
        } else {
            $error = $this->CI->db->error();
            $error_msg = 'Database error: ' . (isset($error['message']) ? $error['message'] : 'Unknown error');
            return ['success' => false, 'error' => $error_msg];
        }
    }

    /**
     * Update check details for an existing record
     * @return array ['success' => bool, 'error' => string or null]
     */
    public function update_check_details($check_id, $check_data) {
        if (!$this->table_exists()) {
            return ['success' => false, 'error' => 'Missing database table: loan_check_details. Run the schema migration or create the table in MySQL.'];
        }
        ...
        $update_data = array(
            'member_id' => $check_data['member_id'],
            'check_type' => $check_data['check_type'] ?? 'applicant',
            'guarantor_id' => $check_data['guarantor_id'] ?? null,
            'check_number' => $check_data['check_number'],
            'bank_name' => $check_data['bank_name'],
            'account_number' => $check_data['account_number'] ?? null,
            'ifsc_code' => $check_data['ifsc_code'],
        ...
        if ($this->CI->db->where('id', $check_id)->update('loan_check_details', $update_data)) {
            return ['success' => true, 'error' => null];
        } else {
            $error = $this->CI->db->error();
            $error_msg = 'Database error: ' . (isset($error['message']) ? $error['message'] : 'Unknown error');
